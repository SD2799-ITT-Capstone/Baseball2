...
function getPlayerInfo($playerID)
{
	if (isset($playerID))
	{
		...
		if($rowCount == 1)
		{
			...
			$playerTable = "Name: " . $row['nameFirst'] . " " . $row['nameLast'] . "<br>\n" ;
		
			if (!is_null($row['birthYear']))
			{
				$playerTable .= "BirtYear: " . $row['birthYear'] . "<br>\n";
			}
			
			if (!is_null($row['birthCity']) && !is_null($row['birthState']))
			{
				$playerTable .= "Birthplace: " . $row['birthCity'] . ", " . $row['birthState'] ."<br>\n";
			}
			
			if (!is_null($row['bats']) && !is_null($row['throws']))
			{
				$playerTable .= "Bats/Throws: " . $row['bats'] . "/" . $row['throws'] ."<br>\n";
			}
			
			if (!is_null($row['debut']) )
			{
				$playerTable .= "Debut: " . RemoveSpacePlus($row['debut']) . "<br>\n";
			}
			
			if (!is_null($row['finalGame']))
			{
				$playerTable .= "Final Game: " . RemoveSpacePlus($row['finalGame']) . "<br>\n";
			}
		}
